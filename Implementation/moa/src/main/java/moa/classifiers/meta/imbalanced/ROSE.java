...
//				ArrayList<Double> accuracies = new ArrayList<Double>();
				ArrayList<Double> kappasAccuracies = new ArrayList<Double>();
...
//					accuracies.add(this.ensemble[i].evaluator.getAccuracy());
					kappasAccuracies.add(this.ensemble[i].evaluator.getKappa() * this.ensemble[i].evaluator.getAccuracy());
				}
...
//					accuracies.add(this.ensembleBackground[i].evaluator.getAccuracy());
					kappasAccuracies.add(this.ensembleBackground[i].evaluator.getKappa() * this.ensembleBackground[i].evaluator.getAccuracy());
				}

				// Select top N classifiers to be the new ensemble, N = ensemble size.
//				for (int i = 0; i < this.ensemble.length; i++) {
//
...
//					this.ensemble[i] = selection.get(i);
					double maxVal = Collections.max(kappasAccuracies);
					int maxIdx = kappasAccuracies.indexOf(maxVal);
					this.ensemble[i] = classifiers.get(maxIdx);
					
					classifiers.remove(maxIdx);
					kappasAccuracies.remove(maxIdx);
				}
...
//				this.warningDetectionMethod.input(correctlyClassifies ? 0 : 1);
				 ...
				 */
				double prediction = correctlyClassifies ? 0.0 : 1.0;
				if (this.warningDetectionMethod instanceof DDM_OCI) {
					((DDM_OCI) this.warningDetectionMethod).input(prediction, instance);
		        } else if (this.warningDetectionMethod instanceof DDM_GMean) {
		        	((DDM_GMean) this.warningDetectionMethod).input(prediction, instance);
		        } else {
...
//				this.driftDetectionMethod.input(correctlyClassifies ? 0 : 1);
				 ...
				 * DDM_OCI is a subclass of DDM_GMean
				 */
				if (this.driftDetectionMethod instanceof DDM_OCI) {
					((DDM_OCI) this.driftDetectionMethod).input(prediction, instance);
		        } else if (this.driftDetectionMethod instanceof DDM_GMean) {
		        	((DDM_GMean) this.driftDetectionMethod).input(prediction, instance);
		        } else {
